...
* 
* 这是一个非常重要的字体选择窗口，非常完整的程序。实现任意保存，直接使用就可以了。
*
*/
...
public class FontChooserWithPole extends JDialog {
 ...
 public FontChooserWithPole(JFrame owner) {
   super(owner, "字体选择窗口", false);
   getContentPane().setLayout(
       new BoxLayout(getContentPane(), BoxLayout.Y_AXIS));

   JPanel p = new JPanel(new GridLayout(1, 2, 10, 2));
   p.setBorder(new TitledBorder(new EtchedBorder(), "字体"));
   ...
   p.setBorder(new TitledBorder(new EtchedBorder(), "样式"));
   ...
   p.setBorder(new TitledBorder(new EtchedBorder(), "预览"));

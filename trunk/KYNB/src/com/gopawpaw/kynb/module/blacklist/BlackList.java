 ...
 * @author LiJinHua
 */
public class BlackList extends BaseModuleFrame implements ActionListener,GppJarRunableInterface{
	...
	private JTable getJTableVillager() {
		if (jTableVillager == null) {
			...
			// 设置列标题(表头)
			mVillagerTableTitle = new String[]{ "身份证号","姓名","黑名单类型","备注信息一","备注信息二","备注信息三"};
	...
	private void setEditRow(int rowS){
		String temp = jTableVillager.getValueAt(rowS,0).toString();
		temp = (temp==null)?"":temp;
		jComboBox_ic.setSelectedItem(temp);
		
		temp = (String) jTableVillager.getValueAt(rowS,1);
		temp = (temp==null)?"":temp;
	...
	private void refreshTableVillagerFromType(String ve_type){
		try {
			ve_type = (ve_type==null || "<查看全部>".equals(ve_type)) ? "":ve_type;
	...
	private GppJComboBox getBlackListTypeComboBox(){
		...
		List<DefultData> list = DataDefultManager.getInstance()
	...
	private JPanel getJPanelDataEdit() {
		...
		jPanelDataEdit0.setLayout(new BorderLayout());
			...
			jComboBox_type = new GppJComboBoxExp(DataDefultManager.getInstance()
					.getDefultData(Villager.tab_ve_type));
			jComboBox_remark1 = new GppJComboBoxExp(DataDefultManager
					.getInstance().getDefultData(Villager.tab_ve_remark1));
			jComboBox_remark2 = new GppJComboBoxExp(DataDefultManager
					.getInstance().getDefultData(Villager.tab_ve_remark2));
			jComboBox_remark3 = new GppJComboBoxExp(DataDefultManager
					.getInstance().getDefultData(Villager.tab_ve_remark3));
			
			
			jPanelDataEdit.add(
					getJPanelNew(Villager.tab_v_ic, jComboBox_ic), null);
			jPanelDataEdit.add(
					getJPanelNew(Villager.tab_v_name, jComboBox_name), null);
			jPanelDataEdit.add(getJPanelNew(Villager.tab_ve_type, jComboBox_type),
					null);
			jPanelDataEdit.add(
					getJPanelNew(Villager.tab_ve_remark1, jComboBox_remark1),
					null);
			jPanelDataEdit.add(
					getJPanelNew(Villager.tab_ve_remark2,
							jComboBox_remark2), null);
			jPanelDataEdit.add(
					getJPanelNew(Villager.tab_ve_remark3,
							jComboBox_remark3), null);
			...
			jPanelButton.setLayout(new BoxLayout(jPanelButton,
					BoxLayout.X_AXIS));
...
//				System.out.println(gcb.getText()+"=-="+e.getStateChange());
	...
	private void actionAddVillager(){
		try {
			...
			kyMap.put(KeyConstants.DB.ve_ic, (String)jComboBox_ic.getSelectedItem());
			kyMap.put(KeyConstants.DB.ve_name, (String)jComboBox_name.getSelectedItem());
			kyMap.put(KeyConstants.DB.ve_type, (String)jComboBox_type.getSelectedItem());
			kyMap.put(KeyConstants.DB.ve_remark1, (String)jComboBox_remark1.getSelectedItem());
			kyMap.put(KeyConstants.DB.ve_remark2, (String)jComboBox_remark2.getSelectedItem());
			kyMap.put(KeyConstants.DB.ve_remark3, (String)jComboBox_remark3.getSelectedItem());

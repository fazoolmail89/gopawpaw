...
//			menuroot.add(temp);
		...
		retJTree
				.addTreeExpansionListener(new javax.swing.event.TreeExpansionListener() {
					public void treeExpanded(
						...
						if (dd3.isHaveChild()) {
							if (!GppMainFrame.actionHistory.lastElement()
									.equals(dd3)) {
								GppMainFrame.actionHistory.add(dd3);
							}
		...
		retJTree.addMouseListener(new java.awt.event.MouseAdapter() {
			public void mouseClicked(java.awt.event.MouseEvent e) {
				...
				// 添加操作历史
				if (GppMainFrame.actionHistory.size() == 0) {
					GppMainFrame.actionHistory.add(dd3);
				} else if (GppMainFrame.actionHistory.size() > 0
						&& !GppMainFrame.actionHistory.lastElement()
								.equals(dd3)) {
					GppMainFrame.actionHistory.add(dd3);
				}
				GppMainFrame.actionHistory2.removeAllElements();
				String tem = sunroot.getFirstChild().toString();
				// 则不需要再次设置该子节点，直接返回
				if (!tem.equals("")) {
